...
namespace Qwaq {
	...
	struct Buffer
	{
		...
		{
			buf = boost::shared_array<unsigned char>(new unsigned char[byteSize]);
			maxSize = usedSize = ((buf == NULL) ? 0 : byteSize);
		}

		bool operator<(const Buffer& other) const { return buf < other.buf; }
		size_t usedSize;
		size_t maxSize;
		boost::shared_array<unsigned char> buf;
	};
	...
	public:
		BufferPool();
		~BufferPool();
		Buffer getBuffer(size_t byteSize);
