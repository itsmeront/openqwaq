...
{ WNDCLASS wc;
  ...
  if(!browserWindow)
    ...
    stWindow = CreateWindowEx(0,
			      ...
			      hInstance,
			      NULL);
    /* In IE protected mode reparenting will not work from here.
       However, it *will* work from control so let's do the work there. */
    PostMessage(browserWindow, g_WM_REPARENT, (ULONG) stWindow, 0);
  }
