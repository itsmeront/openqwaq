...
*/
static sqWin32FileDialog*  dlgFromHandle(int dlgHandle) {
...
*/
DWORD WINAPI runOpenDlg(LPVOID lpParam) {
...
/* runSaveDlg: Run the file save dialog in a separate thread.
   ...
   Return value: Nothing.
*/
DWORD WINAPI runSaveDlg(LPVOID lpParam) {
  sqWin32FileDialog *dlg = (sqWin32FileDialog*)lpParam;
  int ok;

  ok = GetSaveFileNameW(&dlg->ofn);
  if(!ok) {
    /* user canceled or error */
    dlg->errorCode = CommDlgExtendedError();
    dlg->ofn.lpstrFile[0] = 0;
...
/* fileDialogSetCallbackSemaphore: Set the callback semaphore for modal loops.
   ...
   Return value: none.
*/
void fileDialogSetCallbackSemaphore(int semaIndex) {
  /* -- ignored; win32 doesn't need callbacks -- */
}
...
/* fileDialogCallbackReturn: Return from a previous callback.
   ...
   Return value: none.
*/
void fileDialogCallbackReturn(void) {
  /* -- ignored; win32 doesn't need callbacks -- */
}
...
void fileDialogSetLabel(int dlgHandle, char* dlgLabel) {
  ...
  MultiByteToWideChar(CP_UTF8, 0, dlgLabel, -1, dlg->ofn.lpstrTitle, sz);  
}
...
void fileDialogAddFilter(int dlgHandle, char* filterDesc, char* filterPattern){
  ...
  sz = strlen(filterDesc) + strlen(filterPattern) + 3;
  ...
  sz = MultiByteToWideChar(CP_UTF8,0,newFilter,filterStart-newFilter+1,NULL,0);
  if(dlg->ofn.lpstrFilter) free((char*)dlg->ofn.lpstrFilter);
  dlg->ofn.lpstrFilter = calloc(sz, sizeof(WCHAR));
  MultiByteToWideChar(CP_UTF8, 0, newFilter, filterStart - newFilter + 1,
		      dlg->ofn.lpstrFilter, sz);
}
...
void fileDialogSetFilterIndex(int dlgHandle, int index) {
  sqWin32FileDialog *dlg = dlgFromHandle(dlgHandle);
  if(dlg) dlg->ofn.nFilterIndex = index-1;
}
...
int fileDialogSetProperty(int dlgHandle, char* propName, int propValue) {
  ...
  if(strcmp(propName, "oldFilesOnly") == 0) {
    if(propValue) dlg->ofn.Flags |= OFN_FILEMUSTEXIST;
    return 1;
  }
  if(strcmp(propName, "overwritePrompt") == 0) {
...
int fileDialogShow(int dlgHandle, int fSaveAs) {
  ...
  if(!dlg->hThread) {
    MessageBox(0, "Failed to create Thread", "FileDialogPlugin", MB_OK);
    return 0;
  }
  return 1;
}
...
char* fileDialogGetResult(int dlgHandle) {
  ...
  if(!dlg) return NULL; /* fail */
  if(dlg->errorCode != 0) {
    /* FAIL */
    interpreterProxy->primitiveFail();
    return NULL;
  }
  WideCharToMultiByte(CP_UTF8, 0, dlg->ofn.lpstrFile, -1, 
		      result, MAX_PATH, NULL, NULL);
  return result;
}
...
int fileDialogDestroy(int dlgHandle) {
  sqWin32FileDialog *dlg = dlgFromHandle(dlgHandle);
  if(!dlg) return 1; /* doesn't exist; ignore error */
  if(dlg && dlg->hThread) return 0; /* still running */

  if(dlg->filterDesc) free(dlg->filterDesc);
  if(dlg->ofn.lpstrFile) free((char*)dlg->ofn.lpstrFile);
  if(dlg->ofn.lpstrTitle) free((char*)dlg->ofn.lpstrTitle);
  if(dlg->ofn.lpstrFilter) free((char*)dlg->ofn.lpstrFilter);
  if(dlg->ofn.lpstrInitialDir) free((char*)dlg->ofn.lpstrInitialDir);
  dlg->doneSema = 0;
  dlg->used = 0;
  return 1;
}
...
char *fileDialogGetLocation(char *location){
  ...
  static char utf8Result[MAX_PATH]; 
  ...
  if(iniFile == NULL) {
    ...
    iniFile = squeakIniName;
#else
    static char modulePath[MAX_PATH];
    GetModuleFileName(NULL, modulePath, sizeof(modulePath));
    lstrcpy(modulePath + strlen(modulePath)-3, "ini");
    iniFile = modulePath;
#endif
  }

  sz = GetPrivateProfileString("FileLocations", location, "", 
			       utf8Result, MAX_PATH, iniFile);
  if(sz > 0) return utf8Result;

  if(strcmp("temp", location) == 0) {
    GetTempPathW(MAX_PATH, result);
    WideCharToMultiByte(CP_UTF8, 0, result, -1, utf8Result,MAX_PATH,NULL,NULL);
    return utf8Result;
  }

  if(!SHGetFolderPathW) return NULL;

  folder = 0;
  if(strcmp("home", location) == 0) folder = CSIDL_PROFILE;
  if(strcmp("desktop", location) == 0) folder = CSIDL_DESKTOPDIRECTORY;
  if(strcmp("preferences", location) == 0) folder = CSIDL_APPDATA;
  if(strcmp("applications", location) == 0) folder = CSIDL_PROGRAM_FILES;
  if(strcmp("fonts", location) == 0) folder = CSIDL_FONTS;
  if(strcmp("documents", location) == 0) folder = CSIDL_PERSONAL;
  if(strcmp("music", location) == 0) folder = CSIDL_MYMUSIC;
  if(strcmp("pictures", location) == 0) folder = CSIDL_MYPICTURES;
  if(strcmp("videos", location) == 0) folder = CSIDL_MYVIDEO;
