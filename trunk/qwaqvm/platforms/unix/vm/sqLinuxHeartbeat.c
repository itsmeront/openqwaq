/****************************************************************************
*   PROJECT: Linux (setitimer) heartbeat logic for Stack/Cog VM
...
/* Hack for linux to avoid the thread priority issue, i.e. that linux
 * doesn't provide priorities for SCHED_OTHER and won't let a non-superuser
 * process set the scheduling policy to anything else).
 *
...
static int tickCheckInProgress;
	...
	/* Since we are deriving the heartbeat from an interval timer
	 * deadlines are irrelevant and ignored.
	 */
	(void)checkHighPriorityTickees(ioUTCMicroseconds());
...
heartbeat_handler(int sig, struct siginfo *sig_info, void *context)
{
	...
	/* There's a neat idea we could steal from David Lewis and/or John McIntosh,
	 * namely to add pthread signal masking to ensure that forwarded signals are
	 * delivered to the interpreter thread.  But to do this we need the
	 * CogThreadManager to call some code to do this when changing the owning
	 * thread.  Further it would slow down thread switching because it adds a
	 * syste call to switching ownership.  So it may not be worth it.
	 * See UnixOSProcessPlugin>>maskSignalForThisThread:.
	 */
...
extern void *watchdogThreadLoop(void *careLess);

#if TEST_WATCHDOG_THREAD /* make SIGPWR kill the itimer. */
static void
disable_heartbeat_handler(int sig, struct siginfo *sig_info, void *context)
{
	...
	if (setitimer(THE_ITIMER, &itimer, 0))
		perror("disable_heartbeat_handler setitimer");
}

static void
debugSIGDisablesHeartbeat()
{
	struct sigaction disable_heartbeat_action;

	disable_heartbeat_action.sa_sigaction = disable_heartbeat_handler;
	disable_heartbeat_action.sa_flags = SA_RESTART | SA_ONSTACK;
	sigemptyset(&disable_heartbeat_action.sa_mask);
	if (sigaction(SIGPWR, &disable_heartbeat_action, 0)) {
		perror("debugSIGDisablesHeartbeat sigaction");
		exit(1);
	}
}
#else
# define debugSIGDisablesHeartbeat() 0
#endif
...
ioInitHeartbeat()
{
	...
	struct sigaction ticker_handler_action;
	pthread_t careLess;
